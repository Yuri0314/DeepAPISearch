...
def l2norm(tensor):
    ...
    tensor = tensor / (tensor ** 2).sum().sqrt()
    return tensor
# 语义相似度GRU模型
class GRU(nn.Module):
    #其中input_size是指词表的大小
    ...
    def forward(self,input,hidden):
        ...
        output=embedded
        output1,state=self.gru(output,hidden)
        output1=output1[-1]
        # print(l2norm(output1[0]))
        # print(l2norm(output1[1]))
        # output = self.bn(output[-1])
        # output=torch.relu(self.linear1(output1))
        ...
        return self.out(output1).squeeze(dim=0)
...
#小批量计算损失
def batch_loss(rnn_q,rnn_api,query,api,tag,loss, temperature = 0.3):
...
#设置相关超参数
hidden_size,out_size = 128, 128
lr, batch_size, num_epochs =1,2, 20
...
rnn_api = FN(hidden_size,out_size).to(device)
