...
#   初始化GRU的隐藏层状态
    ...
    q_init_state=torch.nn.init.orthogonal_(w, gain=1)
    q_state=q_init_state
    api_init_state=torch.nn.init.orthogonal_(w, gain=1)
    api_state=api_init_state
...
def train(rnn_q,rnn_api,dataset,lr,batch_size,num_epochs,hidden_size,device):
    ...
    for epoch in range(num_epochs):
        ...
        for iter, (query,api) in enumerate(data_iter):
            data_random=Data.DataLoader(dataset,batch_size,shuffle=True)
...
if __name__=='__main__':
    ...
    api_dict = {}
    #设置相关超参数
    hidden_size,out_size = 512, 32
    lr, batch_size, num_epochs =0.005, 2, 50
