...
#小批量计算损失
def batch_loss(rnn_q,rnn_api,query,api,tag,loss,hidden_size, temperature = 0.05):
#   初始化GRU的隐藏层状态
    ...
    # 使用二分类任务
    pre_score=torch.mm(q_output,api_output.permute(1,0))
    cls_prob=torch.flatten(torch.sigmoid(pre_score/temperature))
...
def train(rnn_q,rnn_api,dataset,lr,batch_size,num_epochs,hidden_size,device):
    ...
    step_decay = 0.6
    query_scheduler = torch.optim.lr_scheduler.StepLR(query_optimizer,step_size=8,gamma = step_decay)
    api_scheduler = torch.optim.lr_scheduler.StepLR(api_optimizier,step_size=8,gamma = step_decay)
    ...
    for epoch in range(num_epochs):
        ...
        for iter, (query,api) in enumerate(data_iter):
            ...
            # res_tag=torch.eye(query.shape[0]).to(device)
            #二分类使用tag
            res_tag=torch.flatten(torch.eye(query.shape[0]).to(device))
            l, cls_prob=batch_loss(rnn_q,rnn_api,query,api,res_tag,loss,hidden_size)
            l.backward()
        ...
        print("epoch[{}/{}], loss {:.4f},lr{}".format(epoch, num_epochs, l_sum/len(data_iter),query_optimizer.state_dict()['param_groups'][0]['lr']))
...
if __name__=='__main__':
    ...
    lr, batch_size, num_epochs =0.001, 2, 80
